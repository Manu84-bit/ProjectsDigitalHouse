...
select ProductoID, ProductoNombre, PrecioUnitario from productos where PrecioUnitario between 10 and 22;
select ProductoID, ProductoNombre, UnidadesStock, NivelReorden from productos where UnidadesStock < NivelReorden;
select ProductoID, ProductoNombre, UnidadesStock, NivelReorden, UnidadesPedidas from productos
 ...
 #Clientes
explain clientes;
select clienteID, Contacto, Compania, titulo, pais from clientes order by pais;
select clienteID, contacto, compania, titulo from clientes where titulo = "Owner";
select clienteID, contacto, compania, titulo from clientes where contacto like "c%";
...
select clienteID, facturaID from facturas where ClienteID = "Gourl";
select ClienteID, count(FacturaID) as numeroFacturas from facturas where ClienteID = "GOURL" group by ClienteID;
select clienteID, facturaID, EmpleadoID from facturas where EmpleadoID in(2,3,5,8,9) order by EmpleadoID;
...
SELECT COUNT(facturaID), PaisEnvio FROM facturas GROUP BY PaisEnvio ORDER BY COUNT(facturaID) LIMIT 1;

#Factura detalle
SELECT PRODUCTOID, COUNT(*) from facturadetalle GROUP BY ProductoID ORDER BY count(*) DESC;
SELECT ProductoID, SUM(PRECIOUNITARIO * CANTIDAD) AS TOTAL FROM facturadetalle GROUP BY ProductoID HAVING ProductoID BETWEEN 30 AND 50;
SELECT ProductoID, AVG(PRECIOUNITARIO) AS AVGPU FROM facturadetalle GROUP BY ProductoID;
SELECT ProductoID, PRECIOUNITARIO FROM facturadetalle ORDER BY PrecioUnitario DESC LIMIT 1;

#JOIN 1 Generar un listado de todas las facturas del empleado 'Buchanan'.
SELECT APELLIDO, FacturaID, facturas.EmpleadoID FROM empleados 
INNER JOIN facturas ON empleados.EmpleadoID = facturas.EmpleadoID 
WHERE Apellido = "Buchanan";

#JOIN 2 Generar un listado con todos los campos de las facturas del correo 'Speedy Express'.
SELECT CORREOS.COMPANIA, facturas.FACTURAID, facturas.enviovia FROM CORREOS 
INNER JOIN FACTURAS ON correos.CorreoID = facturas.EnvioVia WHERE Compania = "Speedy Express";

#JOIN 3 Generar un listado de todas las facturas con el nombre y apellido de los empleados.
SELECT FACTURAS.FACTURAID, CONCAT(empleados.NOMBRE, " ", EMPLEADOS.APELLIDO) AS "Nombre y Apellido Empleado" FROM empleados
INNER JOIN FACTURAS ON facturas.EmpleadoID = empleados.EmpleadoID;

#JOIN 4 Mostrar un listado de las facturas de todos los clientes “Owner” y país de envío “USA”.
SELECT FACTURAS.FACTURAID, CLIENTES.COMPANIA, FACTURAS.PAISENVIO, clientes.titulo FROM CLIENTES 
INNER JOIN FACTURAS ON FACTURAS.CLIENTEID = clientes.CLIENTEID WHERE clientes.TITULO = "Owner" and facturas.paisenvio = "USA";

# JOIN 5) Mostrar todos los campos de las facturas del empleado cuyo apellido sea “Leverling” o que incluyan el producto id = “42”.
SELECT FACTURAS.FACTURAID, CONCAT(empleados.NOMBRE, " ", EMPLEADOS.APELLIDO) AS "Nombre y Apellido Empleado", facturadetalle.ProductoID 
FROM FACTURAS
...
#JOIN 6) Mostrar todos los campos de las facturas del empleado cuyo apellido sea “Leverling” y que incluya los producto id = “80” o ”42”.
SELECT FACTURAS.FACTURAID, CONCAT(empleados.NOMBRE, " ", EMPLEADOS.APELLIDO) AS "Nombre y Apellido Empleado", facturadetalle.ProductoID 
FROM FACTURAS
INNER JOIN EMPLEADOS ON facturas.EmpleadoID = empleados.EmpleadoID
INNER JOIN facturadetalle ON facturadetalle.FacturaID = facturas.FacturaID
WHERE EMPLEADOS.APELLIDO = "Leverling" AND facturadetalle.ProductoID IN (42, 80);

# JOIN 7) Generar un listado con los cinco mejores clientes, según sus importes de compras total (PrecioUnitario * Cantidad).
...
INNER JOIN clientes ON clientes.ClienteID = facturas.ClienteID GROUP BY clientes.ClienteID ORDER BY total DESC LIMIT 5; 

# join 8) Generar un listado de facturas, con los campos id, nombre y apellido del cliente, 
#fecha de factura, país de envío, Total, ordenado de manera descendente por fecha de factura y limitado a 10 filas.
SELECT clientes.clienteID, clientes.contacto, facturas.FacturaID, facturas.FechaFactura, facturas.PaisEnvio, sum(facturadetalle.PRECIOUNITARIO * facturadetalle.CANTIDAD) AS TOTAL 
FROM facturas
INNER JOIN facturadetalle ON facturas.facturaid = facturadetalle.FacturaID
INNER JOIN clientes ON clientes.ClienteID = facturas.ClienteID GROUP BY facturas.FacturaID 
...
select facturas.facturaID, facturas.FechaFactura, correos.Compania,
clientes.Contacto, categorias.CategoriaNombre, productos.ProductoNombre,
facturadetalle.PrecioUnitario, facturadetalle.Cantidad from facturas 
...
SELECT categorias.categoriaID, categorias.CategoriaNombre,
productos.productoID, productos.PRODUCTONOMBRE, PRODUCTOS.proveedorID, productos.CANTIDADPORUNIDAD, productos.PrecioUnitario 
FROM CATEGORIAS
...
select clientes.clienteID, clientes.contacto, facturas.FacturaID from clientes 
left join facturas on facturas.clienteid = clientes.clienteID where FacturaID is null;
...
SELECT productos.productoID, productos.PRODUCTONOMBRE, categorias.categoriaID, categorias.CategoriaNombre, PRODUCTOS.proveedorID,proveedores.Contacto, productos.CANTIDADPORUNIDAD, productos.PrecioUnitario 
FROM productos
...
FROM productos
RIGHT JOIN categorias ON categorias.categoriaID = productos.CategoriaID GROUP BY categorias.CategoriaID;

# 5. Para cada cliente, indicar la última factura de compra. Incluir a los clientes que nunca hayan comprado en e-market.
SELECT clientes.ClienteID, clientes.Contacto, max(facturas.FechaFactura) from clientes 
LEFT JOIN facturas ON facturas.ClienteID =  clientes.clienteID GROUP BY clientes.ClienteID;

# 6. Todas las facturas tienen una empresa de correo asociada (enviovia). 
# Generar un listado con todas las empresas de correo, y la cantidad de facturas correspondientes. Realizar la consulta utilizando RIGHT JOIN.
