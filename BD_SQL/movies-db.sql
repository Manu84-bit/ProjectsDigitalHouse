...
select title, rating from movies where title like "%_oy _tory%";
select first_name, last_name from actors where first_name like "Sa%";
select title from movies where release_date between '2004-01-01' and '2008-12-31';

select id, title from movies limit 6 offset 12;
select title as titulo, release_date as lanzamiento, rating from movies 
where rating > 3 and awards > 1 and release_date between '1988-01-01' and '2009-12-31'
order by rating desc;

select title as titulo, rating from episodes order by rating desc;

select id, first_name as nombre, last_name as apellido, rating*10 as puntaje from actors;

select title as titulo from series as seriados;

#Table reference when tables share a key, when the relation betwen tables is many to many, the three tables involve 
#have to be related via the where condition and identity of primary and foreign keys:

SELECT * FROM movies, actor_movie, actors
WHERE movies.id = movie_id AND actors.id = actor_id;

#INNER JOIN

SELECT * from movies inner join genres on genres.id = genre_id;
SELECT * from movies right join genres on genres.id = genre_id;
SELECT * FROM movies 
...
SELECT DISTINCT first_name, last_name FROM movies 
	INNER JOIN actor_movie ON movies.id = movie_id
	INNER JOIN actors ON actors.id = actor_id WHERE title LIKE "%guerra%";

EXPLAIN SERIES;
EXPLAIN genres;
EXPLAIN episodes;
EXPLAIN actor_episode;
explain actors;

SELECT TITLE, GENRES.NAME FROM SERIES 
	INNER JOIN GENRES ON GENRES.ID = GENRE_ID;
    
SELECT TITLE, concat(actors.first_name, " ",  actors.last_name) FROM episodes
	INNER JOIN actor_episode ON episodes.id = actor_episode.episode_id
	INNER JOIN actors ON actors.id = actor_episode.actor_id;
    
SELECT concat(actors.first_name, " ",  actors.last_name), movies.title FROM ACTORS
	INNER JOIN actor_movie ON actor_id = actors.id
    INNER JOIN movies ON movie_id = movies.id
    WHERE movies.title like "%galaxias%";
    
SELECT count(movies.id) AS "Películas por género", genres.name, genres.id as "id de género" FROM movies
