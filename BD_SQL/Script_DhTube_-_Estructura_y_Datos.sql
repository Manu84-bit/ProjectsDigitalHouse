...
-- Database: `DHtube`
--
CREATE DATABASE IF NOT EXISTS `DHtube2` DEFAULT CHARACTER SET latin1 COLLATE latin1_swedish_ci;
USE `DHtube2`;
...
#3
SELECT IDCANAL, FECHACREACION FROM CANAL WHERE FECHACREACION BETWEEN "2021-04-01" AND "2021-06-01";
...
#6
SELECT IDVIDEO, TITULO, CANTIDADLIKES FROM VIDEO ORDER BY CANTIDADLIKES DESC LIMIT 5;

#7

SELECT IDPAIS, NOMBRE FROM PAIS WHERE NOMBRE LIKE "aRG%";

SELECT IDavatar, nombre from avatar where nombre like "%feliz";

INSERT INTO USUARIO (NOMBRE, EMAIL, PASSWORD, FECHANACIMIENTO, CODIGOPOSTAL, PAIS_IDPAIS, AVATAR_IDAVATAR) VALUES
("Juan José Batzal", "jjbatzal@gmail.com", "jj1597", "2000-04-01", "1429", 9, 85);
SELECT * FROM USUARIO WHERE IDUSUARIO=22;


#8
SELECT IDUSUARIO, NOMBRE, FECHANACIMIENTO FROM USUARIO WHERE FECHANACIMIENTO BETWEEN "2000-01-01" AND "2000-12-31";

#9
SELECT IDPAIS, UPPER(NOMBRE) FROM PAIS ORDER BY NOMBRE;

#10
SELECT IDVIDEO, TITULO, CANTIDADREPRODUCCIONES FROM VIDEO WHERE CANTIDADREPRODUCCIONES > 500000;
#n el año 2020 que sean
#privados y que posean más de 100 dislikes.

#11
SELECT IDVIDEO, TITULO, FECHAPUBLICACION, PRIVADO FROM VIDEO 
WHERE FECHAPUBLICACION BETWEEN "2020-01-01" AND "2020-12-31" AND PRIVADO = 1 AND CANTIDADDISLIKES > 100;

#12
UPDATE USUARIO SET FECHANACIMIENTO = "2000-01-04" WHERE IDUSUARIO = 22;

#13
SELECT IDUSUARIO, NOMBRE, PASSWORD FROM USUARIO WHERE length(PASSWORD) <5;

#14
SELECT 
IDUSUARIO AS ID, NOMBRE, EMAIL AS "E-mail" , FECHANACIMIENTO AS "Fecha de Nacimiento", 
TIMESTAMPDIFF(YEAR, FECHANACIMIENTO, NOW()) as Edad, PAIS_IDPAIS AS "Código de país"
FROM USUARIO; 


#15
SELECT MAX(TAMANIO) FROM VIDEO;
SELECT IDVIDEO, TITULO, TAMANIO FROM VIDEO ORDER BY TAMANIO DESC LIMIT 1;
...
DELETE FROM AVATAR WHERE idAVATAR = 84;
